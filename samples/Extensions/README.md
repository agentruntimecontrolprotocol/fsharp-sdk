...
let! _ = request client extTune tunePayload
let! _ = request client extGain gainPayload
let! cap = request client extCapture capturePayload
let! audio = request client extDemodulate
                (JsonDocument.Parse(sprintf """{"iq_artifact_id":"%s",...}""" iqArtifact).RootElement)
```
...
- `Program.fs` — client side: tune, gain, capture, demodulate, then
