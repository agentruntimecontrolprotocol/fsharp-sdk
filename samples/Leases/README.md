...
// returns lease.granted (operator approved) or raises PERMISSION_DENIED.
```
...
- `Program.fs` — opens session, runs one ticket, lease-gates each tool call.
- `Agent.fs` — `LlmStep` discriminated record + `llmLoop` taskSeq stub.
...
- Mirror the `kind: thought` stream into the `Subscriptions` sample
