...
```fsharp
// generator side
let! lease = requestApply client ticketId patch
...
- `IdempotencyKey` per `(ticket, diff)` — §6.4.

## File tour

- `Program.fs` — bounded loop. Two sessions, one process for the demo.
- `Agents.fs` — `Patch`, `ReviewVerdict`, `propose`, `review` — stubs.

## Variations

- Three agents (security + style + correctness reviewers); the runtime
  gates `permission.grant` until all three respond.
