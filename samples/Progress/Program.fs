    ...
    runAsync (fun () ->
        task {
            let! p =
                connect
                    (fun s ->
                        s.RegisterAgent("counter", fun ctx ->
                            task {
                                for i in 0 .. 4 do
                                    do! ctx.EmitProgressAsync(decimal i, Some 4m, Some "step", None, ctx.CancellationToken)
                                return jsonString "done"
                            }))
                    (Set.ofList [ Features.Progress ])
            let! handle = p.Client.SubmitAsync(
                { Agent = "counter"; Input = jsonInt 0
            ...
            try
                ...
                while more do
                    ...
                    if not has then more <- false
                        ...
                        | JobEventBody.Progress(c, t, _, _) ->
                            writeLine (sprintf "progress %O/%A" c t)
                        | _ -> ()
            finally
