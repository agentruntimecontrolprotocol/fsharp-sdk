        ...
        task {
            ...
            let! p =
                connectWithOptions
                    withCredentials
                    (fun s ->
                        s.RegisterAgent("model-user", fun ctx ->
                            task {
            ...
            let! handle =
                p.Client.SubmitAsync(
                    { Agent = "model-user"
                      ...
                      LeaseConstraints = Some { ExpiresAt = DateTimeOffset.UtcNow.AddMinutes 5.0 }
                      IdempotencyKey = None
                      MaxRuntimeSec = None },
            ...
            | [ credential ] ->
                writeLine (sprintf "issued %s for %s" credential.Id credential.Endpoint)
            | other ->
                writeErr (sprintf "expected one credential, got %d" other.Length)
