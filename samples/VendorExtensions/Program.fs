    ...
    runAsync (fun () ->
        task {
            ...
            let! p =
                connect
                    (fun s ->
                        s.RegisterAgent("vendor", fun ctx ->
                            task {
                                // Emit a vendor-extension event the spec doesn't recognise.
                                let body = JobEventBody.XVendor("x-acme.audit", payload)
                                let emit =
                                    typeof<JobContext>.GetField("emit", System.Reflection.BindingFlags.Instance ||| System.Reflection.BindingFlags.NonPublic)
