...
let private errorLine (text: string) = Console.Error.WriteLine ("[arcp] " + text)

let private buildVerifier (token: string option) =
    match token with
    | Some t when not (String.IsNullOrEmpty t) ->
        ARCP.Runtime.Auth.StaticBearerVerifier(readOnlyDict [ t, "developer" ])
        :> ARCP.Runtime.Auth.IBearerVerifier
    | _ ->
        ARCP.Runtime.Auth.DevModeBearerVerifier()
        :> ARCP.Runtime.Auth.IBearerVerifier
...
let private serveStdio (token: string option) : Task<int> =
    task {
        ...
        server.RegisterAgent("echo", fun ctx ->
            task { return Json.serializeToElement<string> "echo" })
...
let private send (url: string) (token: string option) (agent: string) (inputJson: string option) : Task<int> =
    task {
        let! transport =
            WebSocketClientTransport.connectAsync (Uri url) token CancellationToken.None
        let options =
