...
module Registry =
    ...
    module Envelopes =
        ...
        let cancel (p: Cancel) = make (Cancel p)
        let cancelAccepted (p: CancelAccepted) = make (CancelAccepted p)
        let cancelRefused (p: CancelRefused) = make (CancelRefused p)
        let interrupt (p: Interrupt) = make (Interrupt p)
        let backpressure (p: Backpressure) = make (Backpressure p)
        let toolInvoke (p: ToolInvoke) = make (ToolInvoke p)
        let toolResult (p: ToolResult) = make (ToolResult p)
        let toolError (p: ToolError) = make (ToolError p)
        let jobAccepted (p: JobAccepted) = make (JobAccepted p)
        let jobStarted (p: JobStarted) = make (JobStarted p)
        let jobProgress (p: JobProgress) = make (JobProgress p)
        let jobHeartbeat (p: JobHeartbeat) = make (JobHeartbeat p)
        let jobCompleted (p: JobCompleted) = make (JobCompleted p)
        let jobFailed (p: JobFailed) = make (JobFailed p)
        let jobCancelled (p: JobCancelled) = make (JobCancelled p)
        let streamOpen (p: StreamOpen) = make (StreamOpen p)
        let streamChunk (p: StreamChunk) = make (StreamChunk p)
        let streamClose (p: StreamClose) = make (StreamClose p)
        let streamError (p: StreamError) = make (StreamError p)
        let humanInputRequest (p: HumanInputRequest) = make (HumanInputRequest p)
        let humanInputResponse (p: HumanInputResponse) = make (HumanInputResponse p)
        let humanInputCancelled (p: HumanInputCancelled) = make (HumanInputCancelled p)
        let humanChoiceRequest (p: HumanChoiceRequest) = make (HumanChoiceRequest p)
        let humanChoiceResponse (p: HumanChoiceResponse) = make (HumanChoiceResponse p)
        let permissionRequest (p: PermissionRequest) = make (PermissionRequest p)
        let permissionGrant (p: PermissionGrant) = make (PermissionGrant p)
        let permissionDenied (p: PermissionDenied) = make (PermissionDenied p)
        let leaseGranted (p: LeaseGranted) = make (LeaseGranted p)
