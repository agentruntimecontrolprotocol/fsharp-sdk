...
module Codec =
    ...
    let private payloadElement (value: 'T) : JsonElement =
        Json.serializeToElement<'T> value

    let private decodePayload<'T> (env: Envelope) : 'T =
        Json.deserializeElement<'T> env.Payload
    ...
    let writeEnvelope (env: Envelope) : string =
        Json.serialize env
