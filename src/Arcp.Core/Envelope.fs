...
module Envelope =
    ...
    let withSessionId (sid: SessionId) (env: Envelope) : Envelope =
        { env with SessionId = Some sid.Value }

    let withTraceId (tid: TraceId) (env: Envelope) : Envelope =
        { env with TraceId = Some tid.Value }

    let withJobId (jid: JobId) (env: Envelope) : Envelope =
        { env with JobId = Some jid.Value }

    let withEventSeq (seq: int64) (env: Envelope) : Envelope =
        { env with EventSeq = Some seq }

    let withId (id: string) (env: Envelope) : Envelope =
        { env with Id = id }

    let sessionIdOpt (env: Envelope) : SessionId option =
        env.SessionId |> Option.map SessionId.ofString

    let jobIdOpt (env: Envelope) : JobId option =
        env.JobId |> Option.map JobId.ofString
