...
type internal ExpiryWatchdog(timeProvider: TimeProvider) =
    ...
    member _.Start(expiresAt: DateTimeOffset, onExpired: unit -> unit) : unit =
        ...
        let delay = if expiresAt <= now then TimeSpan.Zero else expiresAt - now

        timer <-
            ValueSome(
                timeProvider.CreateTimer(TimerCallback(fun _ -> onExpired ()), null, delay, Timeout.InfiniteTimeSpan)
            )
