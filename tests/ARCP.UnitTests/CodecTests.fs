...
let ``session.hello roundtrips`` () =
    let hello: SessionHelloPayload = {
        ...
        Auth = { Scheme = "bearer"; Token = Some "tok" }
        Capabilities = { Encodings = [ "json" ]; Features = Features.All }
        Resume = None
    }
...
let ``Message.countsInEventSeq excludes ping pong ack`` () =
    let ping = Message.SessionPing { Nonce = "n"; SentAt = System.DateTimeOffset.UtcNow }
    let pong = Message.SessionPong { PingNonce = "n"; ReceivedAt = System.DateTimeOffset.UtcNow }
    let ack = Message.SessionAck { LastProcessedSeq = 1L }
    let evt = Message.JobEvent { Kind = "log"; Ts = System.DateTimeOffset.UtcNow; Body = JobEventBody.Log(LogLevel.Info, "x") }
