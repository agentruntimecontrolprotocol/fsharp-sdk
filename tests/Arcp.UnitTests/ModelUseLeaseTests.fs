...
let ``validateLeaseOp allows model use match`` () =
    let lease =
        Lease.empty |> Lease.withCapability Capabilities.ModelUse [ "tier-fast/*" ]

    match
        Lease.validateLeaseOp lease None Map.empty DateTimeOffset.UtcNow Capabilities.ModelUse "tier-fast/gpt-4o-mini"
    with
    | Ok() -> ()
...
let ``isSubset rejects child model use expansion`` () =
    ...
    let child =
        Lease.empty |> Lease.withCapability Capabilities.ModelUse [ "tier-pro/*" ]
